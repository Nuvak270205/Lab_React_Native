// Câu 1 
function HelloAsyncPromise(): Promise<string> {
...
function getNumberAsyncPromise(): Promise<number> {
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve(10);
        }, 1000);
...
function rejectsAsyncPromise(): Promise<number> {
...
// Câu 4
 function getRamdomNumberAsyncPromise(): Promise<number> {
    return new Promise((resolve, reject) => {
...
async function runSimulateTask(): Promise<void> {
...
async function handleErrorWithTryCatch(): Promise<void> {
    try {
      await rejectsAsyncPromise(); // Promise này sẽ reject
    } catch (error) {
      console.error("Bắt được lỗi:", (error as Error).message); // bắt lỗi và in ra thông báo
    }
}
...
function timeoutPromise(ms: number): Promise<never> {
  ...
  async function fetchUserWithTimeout(id: number): Promise<UserData> {
...
async function postData(data : Todo) : Promise<void> {
    const response = await fetch("https://jsonplaceholder.typicode.com/todos", {
      method: "POST",
      headers: {
...
async function fetchWithRetry(url: string, retries: number): Promise<any> {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.log(`Lần thử ${attempt} thất bại: ${(error as Error).message}`);
        if (attempt === retries) {
...
async function handleAllSettled(): Promise<void> {
    const apiCalls = [
      fetchUser(1),
      fetchUser(2),
